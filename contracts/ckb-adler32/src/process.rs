...
// next index offset
pub const CKBFS_V3_HEAD_NEXT_INDEX_OFFSET: usize = 46; // 5 + 1 + 36 + 4 = 46
...
pub fn load_witnesses_for_ckbfs_v3(index: CKBFS_V3_WITNESSES_INDEX, source: Source) -> Result<CKBFSV3WintessWithMeta, SysError> {
    ...
    let extract_u32 = |offset: usize| -> Result<u32, SysError> {
        if witness.len() >= offset + 4 {
            Ok(u32::from_le_bytes(witness[offset..offset + 4].try_into().unwrap()))
        } else {
    ...
    match index {
        CKBFS_V3_WITNESSES_INDEX::HeadWitness(_) => {
            ...
            Ok(CKBFSV3WintessWithMeta {
                data,
                next_index: if next_index == 0 { None } else { Some(next_index) },
...
pub fn process_ckbfs_validate_v3(args: &[ckb_std::env::Arg]) -> i8 {
    ...
    // Optional recover checksum for append operations
    let recover_checksum = if args.len() > 3 {
        Some(ckb_arg_to_num!(args[3].borrow(), u32))
    } else {
        None
    ...
    let head_witness = match load_witnesses_for_ckbfs_v3(
        ...
        Ok(witness) => witness,
        Err(_) => {
            ckb_std::debug!("CKB-Adler32: Failed to load head witness {}", first_witness_index);
            return CKBFSError::ValidateFailure as i8;
        }
    };
    ...
    while let Some(next_idx) = current_index {
        let witness = match load_witnesses_for_ckbfs_v3(
    ...
    // Use recover checksum from head witness if no external recover checksum provided
    let final_recover_checksum = recover_checksum.or(head_witness.recover_checksum);
    
    // Validate the final checksum
    validate_checksum(expected_checksum, &all_content, final_recover_checksum)
}
